...
Evidence for a person: a free-text reference («طبقات ابن سعد، ص ٩٠») and/or files (scan, photo, PDF). One document (a دفتر العائلة, an إخراج قيد) often backs many people, so **one source is attached to many people** through the «مصدر لـ:» line. Status: built locally (not deployed). Spec: `docs/sources-v1-goal.md`; design + review: `docs/sources-shared-design.md`; threat model: `docs/sources-design-notes.md`; product rules: `docs/prd.md` §5.13.
...
**Copies** (`source-copy.ts#copySources`, called inside each copy transaction right after `persistDeepCopy`, companion of `copyAncestryJumps`). A source travels only with a person or tree that travels. `same` — whole-tree copies inside one workspace (extra-tree duplicate, own-tree collection copy, via `prepareTreeSnapshot`): every source incl. orphans and the tree-wide source, links re-pointed, ciphertext copied as-is. `cross` — every copy into another family (pointer copy, share-token revoke auto-copy, going-private `freezeDependentPointers` / `freezeCollectionLinks`, `copy-borrowed.ts`): only level-3 sources, only links whose person landed AND is not private, a source with no such link is not copied, text / file names / bytes re-encrypted under the target key, no source-family user ids carried. The tree-wide source goes only with whole-tree copies (cross: only at level 3). The target quota never blocks a copy (several copies are automatic): files that don't fit are skipped and counted (`skippedSourceFiles` in the copy result / response), a text-less source that keeps no file is dropped. Copy transactions carrying files use `SOURCE_COPY_TX_TIMEOUT_MS` (60 s).
