...
The workspace detail page lives at `/workspaces/[slug]`. Settings (audit log toggle, version control toggle, umm walad toggle, rada'a toggle, «قفزة نسب» toggle, kunya toggle, branch sharing policy, privacy settings like hide-birth-dates) are admin-only.
...
- Ancestry jumps («قفزة نسب») under `/tree/ancestry-jumps/` — POST / PATCH / DELETE, plus `POST [jumpId]/move-to-new-father` and its undo `POST [jumpId]/move-back` (§4.9)
...
- **Add parent**: button hidden when both parents exist; sex locked to the missing parent when one exists. Server enforces parent-slot uniqueness (409). On a person who carries a «قفزة نسب» it first opens a dialog offering to move the jump to the new (male-locked) father; every other parent-giving route answers 409 `child_has_jump` (§4.9).
...
- **Surname prefill** (patrilineal default, client-side only): `getSurnamePrefill(data, person, formMode)` in `person-detail-helpers.ts` seeds the create form's family-name field. Add child → father's surname (female anchor → husband of the resolved target family, none if no husband); add sibling → father's, else the anchor's; add father → the child's; add mother / spouse / edit → blank. Private or empty/`خاص` surnames are never used. Field stays editable. In add-parent with an unlocked sex choice, `IndividualForm` clears the untouched prefill on أنثى and restores it on ذكر.
- **Move subtree**: replaces the old single-child-between-families move. Updates the person's `FamilyChild` row; descendants follow through their own family chains. Cycle detection runs as a bounded BFS inside the transaction — if the target family's parents are in the moving subtree, reject. The picker (`MoveSubtreeModal`, «تعيين والدين موجودين» / «تغيير الوالدين») labels every parent with the nasab form — name + father + family name (`getDisplayNameWithNasab`) — for existing couples (`getTargetFamiliesForMove`) and free-floating individuals alike; a bare given name is ambiguous, worst for a wife-less family. The picker search matches on the same label.
...
**Workspace toggle.** `Workspace.enableAncestryJumps` (migration `20260923120000_add_enable_ancestry_jumps`, **off by default for every workspace**, no backfill) is an admin-only switch on the workspace settings page («المميزات» → «قفزة نسب»; flips are written to `AdminAccessLog` via the workspace PATCH `AUDITED_FIELDS`). It gates ONLY creating a jump: the `POST` route checks the flag inline (same shape as the rada'a gate) after `requireTreeEditor` + the limiter and before body parsing → 400 «ميزة قفزة النسب غير مفعّلة في هذه المساحة» (no undo-header bypass). `PATCH` and `DELETE` stay open so an existing jump is always fixable and removable (undo of a create and redo of a delete go through `DELETE`). Not gated either: GEDCOM import, every deep-copy / collection-copy / going-private / extra-tree duplicate path, `pruneEmptyAncestryJumps`, and all read paths (canvas edge, nasab, person page, public tree, JSON-LD, export) — turning the feature off hides the button, never the data. Client: the flag is threaded `(canvas)/layout.tsx` → `WorkspaceTreeContext.enableAncestryJumps` → `getAncestryJumpAction(person, data, canEdit, enabled)`, which drops only the `'create'` action (a person who already has a jump still gets `'edit'`, whose sheet carries delete). The «شخص جديد» composite redo rolls back the person + couple it just re-created when the jump POST is refused (e.g. the toggle was turned off since the undo), sharing the forward path's `rollbackDebris`. Dev seeding (`prisma/seed.ts`) switches the flag on for a seeded workspace whose tree carries jumps.

**API** (`/api/workspaces/[id]/tree/ancestry-jumps`; `tree_editor`/admin, `treeMutateLimiter`, treeId-aware via `resolveTargetTreeOr404`): `POST` create (toggle-gated, above); `PATCH [jumpId]` — range + notes only, re-pointing is delete + create; `DELETE [jumpId]` — the jump row only, never the ancestor. Path ids and the DELETE-body `treeId` are uuid-guarded → 404 before anything reaches Prisma. Schemas in `ancestry-jump-schemas.ts` (`MAX_JUMP_GENERATIONS = 200`, notes ≤ 5000). `validateAncestryJump()` (`ancestry-jump-validators.ts`, pure over the resolved tree's own `GedcomData`, so a foreign id 404s server-side) checks in order: J1 descendant exists → J2 ancestor family exists → J3 descendant has no `familyAsChild` → J4 no existing jump → J5 family has ≥ 1 spouse → J6 range ordered (PATCH re-checks on the merged values) → J7 not self → J8 no cycle (`getAllDescendants(..., { includeJumps: true })` must not reach either spouse). A duplicate race surfaces as Prisma `P2002` → the same Arabic 409 (`isDuplicateJumpError`). Responses are a hand-listed DTO with plaintext `notes` (`jumpDto`, `ancestry-jump-route-helpers.ts`).

**Moving a jump to a new father.** `POST [jumpId]/move-to-new-father` (`moveJumpToNewFatherSchema`: `{ treeId?, father }`, `father` = the individual-create fields with `sex: z.literal('M')`; kunya stripped when `enableKunya` is off) — uuid guard → `requireTreeEditor` → limiter → body → `resolveTargetTreeOr404`; **not** gated by `enableAncestryJumps`. The pure planner `planJumpMoveToNewFather(data, jumpId)` 404s an unknown jump, 409s a descendant P who already has parents, then runs `validateAncestryJump` on a PROJECTED tree (placeholder F + couple {husband F, children [P]}, jump re-pointed, `ignoreJumpId`) with the range shifted by `shiftJumpRange` (each stated bound −1, below 1 → unstated — the range counts SKIPPED generations and F is now recorded). One transaction: `SELECT 1 FROM individuals WHERE id = P FOR UPDATE` (a concurrent child insert takes a KEY SHARE lock on P via the `family_children` FK, so it waits — but the families / children POST jump checks run outside a transaction, so a narrow race remains; a DB trigger on `family_children` would close it, follow-up), re-check P has no `familyChild`, create F (`createIndividual` in `src/lib/tree/create-individual.ts`, shared with the individuals POST), create the couple, `ancestryJump.updateMany({ id, treeId, descendantId: P })` → F + shifted range (count 0 → 409 stale; the row is updated IN PLACE — id, encrypted notes, creator kept), three `TreeEditLog` rows via `writeTreeEditLog` (individual create, family create, `ancestry_jump` update with before/after snapshots), then `touchTreeTimestamp`. 201 `{ data: { individual: { id }, family: { id }, jump } }`. Its undo `POST [jumpId]/move-back` (`moveJumpBackSchema`: `{ treeId?, fatherId, familyId, childId, generationsMin, generationsMax }` — the client sends the PRE-move range) is guarded by the pure `checkJumpMoveBack`: the jump sits on F; the couple is {husband F, no wife, children exactly [P]} and no jump's ancestor couple; F has no other family, no parents, no rada link — plus no active branch pointer anchored on F — else 409 «تغيّرت الشجرة منذ هذا الإجراء، فلا يمكن التراجع عنه». Transaction order is load-bearing: jump back to P (+ range) → delete the couple → delete F (the jump FK cascades from its descendant, so F must not go first); three audit rows (F's is the full decrypted row via `getTreeIndividualDecrypted`).

**Parent backstop.** A person who carries a jump may gain recorded parents ONLY through the move route. `ancestry-jump-guards.ts` (`hasJumpDescendant(db, treeId, ids)` — a DB lookup for in-transaction use; `treeHasJumpDescendant(tree, ids)` — in memory over the resolved tree's `ancestryJumps`, used by the two families POST routes; `childHasJumpResponse()` → 409 `{ error: JUMP_BLOCKS_PARENTS_MESSAGE, code: 'child_has_jump' }`, `ChildHasJumpError` for in-transaction use; the code is in `ANCESTRY_JUMP_ERROR_MESSAGES`/`_STATUS`) is called by `tree/families` POST (`childrenIds`), `tree/families/[familyId]/children` POST, the child `move` route (inside its transaction), and `branch-pointers` POST for `parent`, or `sibling` when the anchor has no parent family (inside its transaction — the sibling stitch mints a synthetic parent family). The reverse (creating a jump on someone who has parents) is J3 on the jump POST.
...
**Person page.** In `projectPerson`, `buildPaternalSpine` / `buildFathersChain` resolve father-or-jump (`resolveJumpFather`): the jump-reached `SpineChip` carries `jump: { generationsMin, generationsMax }` and the walk continues up the ancestor's own chain; the boundary and private invariants hold unchanged (a private jump ancestor is the id-less «خاص» placeholder). `projection.ancestryJump` (`{ father, mother, generationsMin, generationsMax }`) is built independently of the spine so a female-only ancestor still surfaces; a private spouse is `null` there, not a placeholder. `PROJECTION_ETAG_VERSION` is `v4`. UI: the shared `JumpDivider` replaces the «بن» token at a jump-reached chip, with one rule per surface — a person's NAME never carries the feature name: `NasabRibbon` and `MotherRibbon`/`MotherDisclosure` render the `connector` variant, «من وَلَد» (comma-bound in the hero ribbon, exactly as `display.ts` punctuates it; no range); the vertical `BloodlineColumn` is not a name but the canvas edge stood on end, so it renders the `chip` variant — the same dashed «قفزة نسب · range» pill as the canvas, labelled by `formatAncestryJumpLabel` — and never badges a jump ancestor «الأب»; `AncestryJumpBlock` (sibling of `RadaBlock`) is the one place a female-only ancestor appears. The hero `NasabRibbon` (member AND public) never carries the family name — with or without a jump; it appears once, in `PersonPage`'s «من بيت X» tag. One-line names from `getDisplayNameWithNasab` keep it: at the default depth 2 a jump is never crossed (output byte-identical, locked by `nasab-depth2-lock.test.ts` over the fixtures); when a deeper chain crosses a jump the frozen surname is placed right BEFORE «، من وَلَد» («باسل بن محمد سعيد، من وَلَد عدنان»), never after the ancestor.
...
**UI entry.** `PersonDetail` offers «قفزة نسب» via `getAncestryJumpAction()` — only when `canEdit`, not `_pointed`, and no `familyAsChild`; with an existing jump the action becomes «تعديل قفزة النسب» (range + notes, delete). `AncestryJumpForm` is the two-path sheet: «شخص جديد» (blank-surname `IndividualForm`) or «شخص موجود في الشجرة» (person picker limited to valid targets → `FamilyPickerModal` when that person has several couples, otherwise a one-spouse family minted silently), then the optional «من»/«إلى» range and notes. Handlers `handleAncestryJumpSubmit` / `Update` / `Delete` in `usePersonActions`. «إضافة والد/والدة» on a person who carries a jump opens `AncestryJumpMoveDialog` first (built on `Modal` + `Button`; names the ancestor, husband ?? wife): «نقل القفزة إلى الأب الجديد» opens the parent form as `{ kind: 'addParent', lockedSex: 'M', moveJumpId }` (branch-link tab hidden), whose submit `handleMoveJumpToNewFather` is ONE move call and ONE undo entry (`buildMoveJumpToNewFatherInverse`: undo = move-back with the current ids + pre-move range, redo = the move again with the saved father form, re-capturing ids; label «نقل قفزة النسب إلى {father}»); «إلغاء» opens nothing. A mother cannot be added this way — the dialog text says to delete the jump manually first. «تعيين والدين موجودين» on such a person shows the same dialog in its block variant, and `moveSubtree` itself refuses before any API call.
...
### 4.10 Sources («المصادر»)

Evidence for a person: a free-text reference («طبقات ابن سعد، ص ٩٠») and/or files (scan, photo, PDF). One document (a دفتر العائلة, an إخراج قيد) often backs many people, so **one source is attached to many people** through the «مصدر لـ:» line. Status: deployed 2026-09-26 (`669274d`). Product rules, status and follow-ups: `docs/prd.md` §5.13.

**Data model.** Migrations `20260925120000_add_source_entries`, `20260926120000_source_file_staging`, `20260927120000_shared_sources` (the last one hand-edited to backfill: each old per-person entry became one source + one link). `SourceEntry` (`source_entries` — the model keeps its old name; it is a "source" in product terms): `treeId`, `isTreeWide`, `visibility` (`SourceVisibility`: `admins` = level 1 «المشرفون فقط» (default) / `members` = level 2 «أعضاء مساحة العائلة» / `public` = level 3 «أعضاء مساحة العائلة وزوار الشجرة المنشورة»; plaintext), `text` (`Bytes?`, AES-256-GCM under the workspace key), `createdById`. `SourceLink` (`@@id([sourceId, individualId])`, `treeId`, `createdById`): deleting the person or the source cascades the link; the **source survives its people** — a source with zero links is «ليس مصدرًا لأحد» (admin-only). The tree-wide source («مصدر الشجرة», one per tree, no links, shown as «من مصدر الشجرة» on a person with no own visible source) is the explicit `isTreeWide` flag, never inferred from having no links. `SourceFile` holds metadata only (encrypted `fileName`, `mimeType`, `sizeBytes`, `entryId` — null while staged); `SourceFileData` holds the encrypted bytes one-to-one, so no `include: { files: true }` ever pulls contents and the tree loader never touches it. Hand-added SQL: partial unique index `source_entries_one_tree_wide_per_tree … WHERE is_tree_wide`, CHECKs on `size_bytes` (1..8 MB) and `mime_type` (JPEG/PNG/WebP/PDF), partial index `source_files_staged_created_at … WHERE entry_id IS NULL`. App-enforced and tested: a tree-wide source has no links; a link's person and source share its `treeId`.

**The gate.** `src/lib/tree/source-visibility.ts` is the ONE place that interprets a level; pure, fail-closed (unknown level / viewer / malformed person → `false`). The decision is per **(source, person)**: `canViewSourceEntry(source, person, viewer)` — admin sees every level, including on private people ("admins uploaded it"; the tree payload shows private people as «خاص» even to admins, so admins reach them through the per-person route); member sees levels 2–3, never on a private person; public visitor sees level 3 only on a person the public tree shows. `_pointed` (borrowed) people show no sources to anyone — sources are served only by the owning workspace. `visibleLinkedPeople` is the only input for names, counts and the «مشترك مع» tag (a count never includes a hidden person); Source DTOs never expose a link the viewer can't see. `canViewSourceAnywhere` (admin always, incl. orphans; others: tree-wide gate or ≥ 1 visible linked person) guards the member file route, the preview and suggestions; `inheritedTreeEntry` decides «من مصدر الشجرة». Non-admins may only create at level 1 and never link a private person — refused with the same generic 400 as a nonexistent, foreign-tree or borrowed person.

**Never in the tree.** Sources are not part of the tree GET payload, `GedcomData`, the person projection, SSR, OG, the sitemap or JSON-LD; they are fetched only through their own routes, so they never touch `lastModifiedAt`/ETags. `stripSourceKeys` (`source-key-strip.ts`) is the fail-closed backstop on `redactForPublic`, `redactPrivateIndividuals` and `extractPointedSubtree`; the leak tests' denylist (`src/test/helpers/source-leak.ts`) covers `sourceLinks`, `people`, `sharedCount`, `peopleCount`, `personIds`. Every "not visible" case — malformed id, nonexistent, another tree/workspace, gated out, staged file — answers ONE identical 404 «غير موجود» (public: the public tree's 404 body), all with `Cache-Control: private, no-store`.

**Member API** (`/api/workspaces/[id]/tree/…`, treeId-aware via `resolveSourceTreeOr404`, `treeMutateLimiter` on writes). `GET individuals/[id]/sources` returns the viewer's entries with `people` (visible OTHERS, ≤ 20 names) and `sharedCount`, the inherited tree-wide source, and for editors on a person with no sources up to 2 `familyHints` (sources linked to the household — parents, spouses, siblings, children — most-linked first). `POST sources` creates a source + its links (1..500 people, all native to this tree) + attaches staged files in one transaction. `PATCH sources/[entryId]` carries content and link deltas in ONE call (source row `FOR UPDATE`); if it would leave a non-tree-wide source with zero links it needs `onLastLink` — missing → 409 `code: 'last_link'`, `delete` → source and files gone, `keep` → orphan. A non-admin's remove list silently ignores private ids (no "is this person linked?" oracle). `DELETE` by an admin removes the whole source; by a non-admin editor with hidden links remaining, only the visible links (200 `{ deleted: false }`). `GET sources/[entryId]` is the preview; `GET sources/suggestions` gives ≤ 10 gate-filtered summaries for the typing box; the admin list (`GET sources`) is one row per source with `filter` (`shared` = ≥ 2 people / `unlinked`) and tab counts; `POST sources/bulk` changes level or deletes per source; `GET/PUT/DELETE sources/tree-entry` manages the tree-wide source. Audit: `entityType: 'source_entry'` («مصدر»), `snapshotSourceEntry` (level, text inside the encrypted snapshot, `fileCount`, `personIds` + `peopleCount`), link changes logged as `update` with `linksAdded`/`linksRemoved`; never source text in the description, never file bytes.

**Files.** Upload is staged: `POST sources/uploads` takes ONE file per request (`sourceUploadLimiter`, 30/h per user) and stores it with `entryId = null`; a create/PATCH/PUT attaches it via `fileIds` (only the uploader's own fresh files in the same tree; ≤ 20 per source, checked under the entry row lock). `Content-Length` is required and capped before any byte is read, and the stream is capped again (a lying header can't smuggle more). The type is decided by magic bytes only. Images are decoded and re-encoded with `sharp` in their own format (`.rotate()` applies EXIF orientation; no `withMetadata()`, so EXIF/GPS/ICC/XMP never survive; `limitInputPixels` ≈ 50 MP). PDFs are stored as-is but refused when they carry active content (JavaScript, auto-actions, launch, embedded files — also inside Flate streams and behind `#xx` name escapes), are encrypted, or have any stream filtered by anything other than a single `/FlateDecode` (hex/ASCII85/LZW, filter chains, indirect filter refs — the scan can't see through them); image codecs (`DCT`/`JPX`/`CCITTFax`/`JBIG2`, alone or after one Flate) are allowed on image XObjects only, so scanned PDFs pass. Cap 8 MB (matches the DB CHECK). The workspace `storageQuotaBytes` is checked under a workspace row lock so two uploads can't both pass; the admin dashboard's storage figure includes source files. Each upload best-effort sweeps staged files older than 24 h (`sweepStagedSourceFiles`, cheap via the partial index). Serving (`GET sources/[entryId]/files/[fileId]`, member + `canViewSourceAnywhere`) decrypts only after the gate passes and sends `X-Content-Type-Options: nosniff`, `Content-Security-Policy: sandbox; default-src 'none'`, `Cross-Origin-Resource-Policy: same-origin`, `no-store` and a sanitized `Content-Disposition`. The client fetches files with the Bearer token and shows object URLs (`useSourceFileUrls`, revoked on change/unmount). File upload/removal is not undoable.

**Public serving** (`src/lib/tree/public-sources.ts`, anonymous, deny-by-default, `publicTreeLimiter`, `X-Robots-Tag: noindex`). A visitor sees a source only when the tree is published (the same `loadPublicTreeBySlug` loader), the person is in the SAME redacted public payload (`buildPublicTreePayload`) as a HOME person with `publicDisplay === 'full'` (a private person is `redacted`, a presumed-living one `living` — both hide their sources), and the gate passes with viewer `public`. Routes: `GET /api/family/[slug]/person/[individualId]/sources` (DTO exactly `{ id, text, files }` — never people, counts or the shared tag), `…/sources/[entryId]/files/[fileId]` (the file's source must be LINKED to the path person, so a shown person can't be used to fetch another person's file) and `GET /api/family/[slug]/sources/tree-entry/files/[fileId]`. `PublicPersonSourcesCard` renders them client-side on the public person page and the public tree panel. `isShownOnPublicTree` (`public-shown.ts`: not private, not `_pointed`, not presumed living) is the client/server mirror used by the level-3 warning and the publish summary, parity-tested against `redactForPublic`.

**Publish step.** The publish flow (`PublishFlowContainer` → `PublishSourcesStep`) asks once, over sources at levels 1–2: «كلها» / «لا شيء» (default) / «أختار بنفسي»; the choice is applied as bulk raises to level 3 (`publish-sources.ts`, chunks of 500 over `publish-summary`'s uncapped `pendingIds`). The already-public line shows how many people those sources appear on (`publicPeopleCount`, counts only). The tree-wide source is offered separately.

**UI.** `src/components/sources/`. Sidebar `PersonSourcesSection` (rows with 32px thumbs, «للمشرفين» badge on level 1, green «مشترك مع K آخرين» tag, «⋯» menu: «تعديل» / «إزالته عن {name} فقط» / «حذف المصدر من الجميع (N)»; `FamilySourceHints` «مصادر أسرته»); member person page `PersonSourcesCard` (tag only, no names). `SourceEntryForm` fields: «المصدر» · «ملفات» · «مصدر لـ:» (starting person = locked chip, «هذا الشخص (جديد)» in create modes; > 4 chips collapse) · `SourceVisibilityPicker` (3 levels + the tree's live publish-status line via `useTreePublishLevel`; level-3 files warning, extended with «(S من N)» shown people when shared). Editing a shared source shows «هذا المصدر لـ N أشخاص…» and «حفظ عند N أشخاص» when content changes (wording from `peopleCountLabel`). `SourcePeoplePicker` offers quick buttons (spouse(s)/children/whole family from a parent; parents/siblings/whole family from a child) and same-tree search; rows show nasab + relation (`relationLabel`) + birth; private people are never offered (the tree shows them as «خاص» even to admins — an admin links a private person from that person's own panel), borrowed people are listed but not selectable. Reuse never re-uploads: suggestions and family hints link an existing source ([ربطه بهذا الشخص]) or copy its text ([نسخ النص فقط]); «عرض» opens `SourcePreview`. Removing a source's last person opens `LastLinkDialog` ([حذف المصدر وملفاته] / [إبقاؤه في صفحة المصادر]; ✕/Esc cancels). `IndividualForm` (edit and add child/spouse/parent) stages source changes (`StagedSourcesList`, `source-staging.ts`) and nothing is saved before its «حفظ»; `applySourcePlan` then runs `creates`/`links`/`updates`/`unlinks`/`deletes`, swapping the `SOURCE_SELF` placeholder for the new person's id. Stacked modals close top-first on Esc/backdrop (`modalStack.ts`). Admin page `/workspaces/[slug]/tree/sources` (admin-only; toolbar link admin-only): `TreeSourceCard` then `SourcesManager` — tabs «الكل / مشترك / ليس مصدرًا لأحد», level filter, search, per-row people with × and «＋ إضافة أشخاص», «تحديد الكل» over every match, bulk level change / delete (per source).

**Deleting a person.** Their sources are kept: links cascade, a source whose last person is gone becomes «ليس مصدرًا لأحد». The delete confirm tells admins so when one of the person's sources has no other person. Cascade delete behaves the same (not undoable).

**Undo** (`source-entry-undo.ts`, `undo-builders.ts`). A form save (source + links, incl. reuse links) is ONE entry — «إضافة مصدر» / «إضافة مصدر لـ {N}» / «تعديل مصدر لـ {N}»; link inverses are PATCH add/remove; a save involving files is `undoOnly` (files are never re-creatable). «إزالته عن {name} فقط» is one entry (undo = PATCH `addPersonIds`); if it deleted the source it is undoable only when text-only. Delete-from-everyone: text-only → undo re-creates with the same people; with files → not undoable. Person delete captures the person's source ids and its undo re-links them to the re-created person (text AND files come back; a since-deleted source is skipped). Uploads, file removal, admin-page and bulk actions are not undoable. Labels never contain source text.

**Copies** (`source-copy.ts#copySources`, called inside each copy transaction right after `persistDeepCopy`, companion of `copyAncestryJumps`). A source travels only with a person or tree that travels. `same` — whole-tree copies inside one workspace (extra-tree duplicate, own-tree collection copy, via `prepareTreeSnapshot`): every source incl. orphans and the tree-wide source, links re-pointed, ciphertext copied as-is. `cross` — every copy into another family (pointer copy, share-token revoke auto-copy, going-private `freezeDependentPointers` / `freezeCollectionLinks`, `copy-borrowed.ts`): only level-3 sources, only links whose person landed AND is shown on the public tree (`source-link-shown.ts`: not private, not presumed living — so living people's documents stay behind), a source with no such link is not copied, text / file names / bytes re-encrypted under the target key, no source-family user ids carried. The tree-wide source goes only with whole-tree copies (cross: only at level 3). The target quota never blocks a copy (several copies are automatic): files that don't fit are skipped and counted (`skippedSourceFiles` in the copy result / response), a text-less source that keeps no file is dropped. Copy transactions carrying files use `SOURCE_COPY_TX_TIMEOUT_MS` (60 s).

**GEDCOM.** Sources are not imported or exported in v1. Import counts dropped `SOUR`/`OBJE` (records, INDI/FAM and event level; `countSkippedSources`) and returns `skippedSources`; the editor shows an 8-second notice. Export/import of sources is v2.
...
### 5.6 Collection-link pointers (cross-workspace add-by-link)

Collections "add by link" (Chunk 3) reuses `BranchPointer` as a SOURCE DESCRIPTOR, not a member-tree stitch. `resolveLinkSource()` (`src/lib/collections/resolve-link.ts`) turns a pasted public-tree slug or a private share code into one `ResolvedLinkSource | null` — IDENTITY only: the source workspace + the SPECIFIC published tree id, **main OR extra** (a published extra tree is borrowable too; a non-public slug → null). The route then branches:
- **self-source** — the resolved tree is in the *caller's own* workspace (`source.sourceWorkspaceId === workspaceId`, where `workspaceId` is the validated collection workspace). The link just names one of your own trees, so it short-circuits to the shared `addOwnTreeLinked` helper — a live linked item, `linkMode` ignored (own-tree adds are always live links), and NO reuse gate (reuse is a cross-workspace concept). A re-add → the same tree-dup 409 the direct picker gives.
- **cross-workspace** — the LIVE `allowReuse` reuse-gate fires HERE (in the route, not the resolver, so a self-source own paste is never reuse-blocked); every cross-workspace failure (unknown/expired token, unknown/private/non-reusable slug) returns the SAME generic 404 (no enumeration oracle) behind a per-IP limiter. The borrow/copy paths fetch the source tree BY id (`getTreeByIdWithIncludes(workspaceId, treeId)` — workspace+id scoped, fail-closed) so the SPECIFIC published tree (main or extra) is borrowed, never the workspace main by default.

- **linked** → an ANCHOR-LESS pointer with `isCollectionLink: true` and null `anchorIndividualId`/`selectedIndividualId`/`relationship`. It has no person in the target tree to graft onto, so it must never enter the member-tree merge. The explicit `isCollectionLink` column (NOT inferred from a null anchor) is the fail-closed discriminator: `getActivePointersForWorkspace` + `isPointedIndividualInWorkspace` (covering the member tree GET, GEDCOM export, and the 5 mutation guards), the GET pointers list, the `MAX_POINTERS` cap count, the public borrowed-subtree + withheld-branches queries, and the member disconnect all filter `isCollectionLink: false`. Source-side readers (going-private freeze, token-revoke auto-copy, admin takedown) still see them. Token-revoke EXCLUDES collection links (no anchor to stitch into the target's main tree) — a revoked source is hidden by the serve-time reuse-gate, reappear-on-republish. Going-private now **preserves** them (Chunk 4): `freezeCollectionLinks` deep-copies each affected borrow into a new frozen `extra` tree and re-points its `CollectionItem` to that copy (skipping borrows whose source leaf is still public+reusable). `isStitchablePointer()` (`branch-pointer-guards.ts`) centralizes the "real anchored pointer" narrowing in both deep-copy loops.
- **copied** → `copyBorrowedBranchIntoNewExtraTree()` (`copy-borrowed.ts`): a TWO-KEY deep-copy (decrypt under the source key, re-encrypt under the target key) into a new `extra` tree, with `CopyProvenance` (reason `collection_link_copy`) written in the same transaction. Bound via `treeId`. No ciphertext crosses the workspace boundary.

GEDCOM export fetches source workspace keys in parallel and decrypts each merged subtree before serializing.

### 5.7 Public collection serving (Chunk 4)

A published collection serves anonymously at `/collections/[slug]` — its own deny-by-default surface, separate from both the member path and the per-tree public path. `src/lib/collections/public-serve.ts` is the serve layer: `loadPublicCollectionBySlug` / `getPublicCollectionForRequest` (`cache(...)`) resolve the collection by its `publicSlug` (unknown / private / `enableCollections`-off → null → generic 404), and `buildPublicCollectionPayload` returns a LIGHTWEIGHT card payload (`{ titleAr, descriptionAr, trees: { slug, titleAr, peopleCount }[] }`) — no tree data is inlined; each card links to that tree's own `/family/[slug]` viewer, which redacts once at its own page (so the redactor is never duplicated here).

`collectPublicTreeRefs` walks the collection with the same bounded visited-set + `MAX_NESTING_DEPTH`/`MAX_ITEMS` discipline as the cycle guard, recomputing **LIVE** effective visibility per item (`resolveEffectiveVisibility`): only genuinely-public trees/branches become cards; nested collections recurse without ever widening a child past its own visibility; cross-workspace borrows additionally re-check the live source `allowReuse`; a surviving leaf with no current `publicSlug` is dropped (fail-closed). The route (`GET /api/collections/[slug]/tree`) applies `publicTreeLimiter` + `clientIpKey`. The SSR page (`/collections/[slug]`, `PublicCollectionPageClient`) renders a brand-faithful (`/design-preview` vocabulary) hero + glass card grid. **Search discoverability (2026-06-18):** the page is `noindex` unless the collection is `public_listed` AND fully-listable — every servable leaf tree, recursively, is itself `public_listed` (a link-only leaf blocks listing, so an unlisted family is never surfaced in search through a collection). That gate is `getCollectionListingReadiness(slug)` (a `cache(...)` over the shared `computeListingReadiness` core); when it passes the page gets OG + `CollectionPage`/`BreadcrumbList` JSON-LD and a sitemap entry (`listIndexableCollectionSlugs()`). At publish time `CollectionVisibilityModal` shows any not-yet-listed leaves split into own-workspace trees (one-click "list them all" via `promoteOwnTreesToListed` on the visibility PATCH) vs cross-workspace borrows (the owner must ask the source family or remove them); if borrowed leaves still block, the publish is downgraded to `public_link` with a clear by-link-only notice.

Publishing a collection (`PATCH .../collections/[id]/visibility`, admin-only) mints the slug on first publish and keeps it across private round-trips; it requires NO type-to-confirm phrase (each tree inside was already confirmed at its own publish, and private trees are never served). *Known cleanups (deferred):* the publish "what's withheld" warning computes withholding client-side with a rule that omits the cross-workspace `allowReuse` gate (errs safe — withholds more than warned, never leaks); the authoritative `getCollectionPublishPreview` endpoint exists but the modal doesn't yet consume it. `freezeCollectionLinks` is also wired into this route where it is currently a no-op (a collection going private changes no source tree's visibility).
...
- **SEO / search discoverability** — the page server-renders the family name + crawlable names list. A `public_listed` main tree is genuinely search-discoverable: `index,follow`, an Arabic OG card + `WebPage`/`BreadcrumbList` JSON-LD, and an entry in the sitemap. Everything else (`public_link`, any `extra` tree) is `noindex` and absent from the sitemap. The index/noindex gate is the single `isPublicTreeIndexable(record)` predicate; the sitemap source `listIndexableTreeSlugs()` mirrors it. Crawlable content derives only from the post-redaction payload (living birth dates already hidden server-side). Every inline JSON-LD `<script>` is serialized with `safeJsonLd` (`src/lib/utils/safe-json-ld.ts` — escapes `<`, `>`, `&`, U+2028/9), never bare `JSON.stringify`.
- **Per-person pages (reverses the earlier "no per-person pages" rule)** — every non-private person on a published tree has a readable page at `/family/[slug]/person/[id]`, served for by-link and listed trees alike. Search exposure is a separate, stricter, OFF-BY-DEFAULT opt-in: a person page is `noindex` and carries no `Person`/genealogy JSON-LD unless the tree is `public_listed` AND the owner ticked `personPagesIndexable`. That one predicate, `isPublicPersonPageIndexable(record)`, gates all four consumers: the robots meta, the `Person` JSON-LD (`person-jsonld.ts`, dateless + per-node redaction check), the name links in the tree page's crawlable list, and the person URLs in the sitemap (`listIndexablePersonUrls()` mirrors it as SQL, capped at `MAX_SITEMAP_PERSON_URLS`; sharding the sitemap is deferred). The page title is the person's two-generation nasab plus the family name, built from the already-redacted payload so a private ancestor contributes the «خاص» placeholder, never a real name.
- **Make-private** — `going-private.ts` stops serving and converts any live anchored links from other collections into frozen copies (`freezeDependentPointers`); the slug is kept. Anchor-less collection-link borrows are preserved separately by `freezeCollectionLinks` (Chunk 4 — see §5.6, §5.7).
- **Standalone extra-tree publish (Collections Chunk 3)** — `PATCH .../tree/visibility` accepts an optional `treeId` to publish a single `extra` tree on its own; admin-only, gated by `enableCollections`. It serves HOME-ONLY (no borrowed branches), always `noindex` (only the `main` tree is ever search-discoverable), through the same `redactForPublic` + living-birth-date rules. Unpublishing an extra tree does NOT run the workspace-scoped going-private freeze (it would break the main tree's dependent collections).
- **Unified publish UI** — ONE `PublishFlowContainer` (`src/components/public-tree/`, treeId-aware: `{ workspaceId, treeId?, familyName, onClose, onChanged? }`) drives publishing for EVERY tree, behind one shared `PublishIcon`: the editor top bar (publishes the *open* tree — main or extra — via `activeTreeId`) AND every row on the trees screen (main + extra). `publish-preview` is treeId-aware (`resolveTargetTreeOr404`; living-people checkpoint, the tree's own name as the confirm phrase, withheld-branches `[]` for extra trees). There is no separate extra-tree publish modal — the single component is the architectural guarantee that the two entry points can't diverge.
...
- RadaFamily: `notes`
- AncestryJump: `notes`
- SourceEntry: `text`; SourceFile: `fileName`; SourceFileData: the file bytes (`encryptBytes`/`decryptBytes`) — see §4.10
...
**Inverse builders** (`src/lib/tree/undo-builders.ts`) cover 16 operations: individual create/update/delete, family create/update/delete, family children add/remove/move, rada'a family create/update/delete, ancestry jump create/update/delete and move-to-new-father (the «شخص جديد» jump path pushes one hand-rolled composite entry; the move is one entry whose undo is the move-back route, see §4.9). Sources have their own inverses (`source-entry-undo.ts`; one entry per form save, person-delete undo re-links sources — see §4.10). Labels come from `buildUndoLabel()` (gendered variants, 40-char truncation).
...
`POST /tree/import` — tree_editor only, empty tree only (409 otherwise), multipart form, 7 MB file limit, 10K record cap. Seed helpers are reused for RadaFamily / RadaFamilyChild and AncestryJump creation; the response reports `ancestryJumpCount` beside `radaFamilyCount`. Sources are not imported: the response reports `skippedSources` (dropped `SOUR`/`OBJE`) and the editor shows a notice (§4.10). Import is intentionally not Ctrl+Z-undoable.
