...
**Status**: **Public Tree v1 — ✅ shipped (2026-06-16).** **Collections — Chunk 1 ✅ shipped (2026-06-17); Chunks 2–4 remaining (2=edit extra-tree content, 3=add-by-link, 4=public serving).** Design decisions for both captured below.
...
2. **Collections** — partially shipped, built in chunks:
   ...
   - **Chunk 2 — NEXT.** Edit extra-tree *content*: teach the existing tree editor to open ANY tree in the workspace (by `treeId`), not just the main one, so an extra tree gets the same add / edit / delete people + relationships controls. The whole point of extra trees (§2.5 — documenting many trees) depends on this; without it they're empty shells you can only create/rename/duplicate. Contained, no new privacy or cross-workspace boundary — `tree_editor`/`collection_editor` gating, same per-workspace encryption path as the main tree.
   - **Chunk 3.** Cross-workspace add-by-link (borrowed public trees / privately-shared branches) reusing the share-token + deep-copy machinery, with the live `allowReuse` reuse-gate; standalone extra-tree publishing (home-only serve, §7 correction).
