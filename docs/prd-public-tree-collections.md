...
**Status**: **Public Tree v1 — ✅ shipped (2026-06-16).** **Collections — Chunks 1–4 ✅ shipped + cleanup pass ✅ done (2026-06-18). Feature fully complete — no outstanding cleanups.** **Search discoverability — ✅ shipped (2026-06-18):** a `public_listed` main tree is now genuinely search-discoverable (OG + `WebPage`/`BreadcrumbList` JSON-LD + sitemap entry; no `Person`/genealogy schema — would leak redacted living PII); a `public_listed` collection is discoverable only when fully-listable (every servable leaf tree is itself `public_listed`) — a link-only leaf keeps the collection `noindex`, and the publish modal lets the owner one-click "list all my trees" (`promoteOwnTreesToListed`) while flagging cross-workspace borrowed leaves that block it (collection downgrades to `public_link` if unresolved). `public_link` / `extra` trees stay `noindex`/link-only. **Open follow-ups (deferred to a clean session):** (a) whether to hide living people's *names* from the crawlable names list (today a listed tree exposes living names + relations, hiding only exact birth dates — owner warned at publish); (b) per-tree OG/social-share image (currently falls back to the site-wide image). **Owner decision (2026-06-18): deploy this whole Public-Tree + Collections + search batch to production BEFORE starting any new feature (Deep Citation), then email all users announcing Public Tree + Collections.** Design decisions for all three captured below.
...
- **Separate readable per-person / family pages** — originally DROPPED in favor of the lighter approach in §7.6; **since shipped for per-person pages** (see the amendment in §7.6). Every non-private person on a published tree has a readable page, and a friendly per-tree checkbox — off by default — decides whether those pages may enter search. A separate family overview page is still dropped.
...
2. **Collections** — partially shipped, built in chunks:
   ...
   - **Chunk 3 — ✅ DONE (2026-06-17, commit f294cdd).** Cross-workspace add-by-link + standalone extra-tree publishing. A workspace pastes a public-tree link or a private branch share code to add another family's tree/branch as a collection item — `linked` (a live, anchor-less collection pointer; zero bytes copied) or `copied` (TWO-KEY cross-workspace deep-copy: decrypt under the source key, re-encrypt under the target key into a new extra tree, with `CopyProvenance` written in-transaction). The live `allowReuse` reuse-gate is re-checked at add-time; every resolve failure (unknown/expired token, unknown/private/non-reusable slug, self-source) returns ONE generic 404 — no enumeration oracle — behind a per-IP limiter. A new explicit `BranchPointer.isCollectionLink` discriminator (with anchor/selected/relationship made nullable) keeps these source-descriptor pointers fail-closed-excluded from every member-tree and public reader (merge, GEDCOM export, the 5 mutation guards, the pointer list, the public borrowed-subtree + withheld-branches queries, the MAX_POINTERS cap, the member disconnect) while source-side freeze / token-revoke / admin-takedown still see them. A single `extra` tree can now be published on its own (home-only serve — no borrowed branches — always `noindex`, behind `enableCollections`). On a source going private, collection links are left untouched (excluded from the freeze) and hidden by the serve-time reuse-gate — reappear-on-republish; the full freeze-repoint preservation is Chunk 4. Security-reviewed: cross-tenant and encryption boundaries closed, no critical/high. *Two deferred low-severity follow-ups:* the `MAX_EXTRA_TREES` cap is not enforced on the copy paths (rate-limited; pre-existing, widened to cross-tenant here); one cosmetic publish edge.
   - **Chunk 4 — ✅ DONE (2026-06-18).** Public collection serving. A published collection now serves at its own deny-by-default anonymous route (`/collections/[slug]`, gated by `enableCollections`, generic-404 + per-IP limiter, always `noindex`) — a brand-faithful card grid of the collection's public trees, each card linking out to that tree's own `/family/[slug]` viewer (no tree data inlined; each tree redacted once at its own page). The serve layer (`src/lib/collections/public-serve.ts`) walks the collection with the same bounded visited-set/depth caps as the cycle guard and recomputes **LIVE** effective visibility per item, including only genuinely-public trees/branches and recursing into nested collections without ever widening a child past its own visibility (cross-workspace borrows also re-check the live source `allowReuse`). Going-private now **preserves** collection-link borrows: `freezeCollectionLinks` deep-copies each affected borrow into a new frozen extra tree and re-points the `CollectionItem` (replacing Chunk 3's reappear-on-republish behavior). The collection publish/visibility flow + warning ship (admin-only `PATCH .../collections/[id]/visibility`). Security-reviewed: no critical/high; the public-exposure boundary is sound. **Decision:** publishing a collection intentionally requires NO type-to-confirm phrase (each tree inside was already confirmed at its own publish; private trees are never published). *Deferred low-severity cleanups (next "cleanup session"):* (1) `MAX_EXTRA_TREES` cap not enforced on the copy paths (`copy-borrowed.ts`/`copy.ts`); (2) two-tx crash-window in `copyBorrowedBranchIntoNewExtraTree`; (3) the publish "what's withheld" warning computes withholding client-side with a rule that omits the cross-workspace `allowReuse` gate the serve path applies — it errs SAFE (withholds more than it warns, never leaks) but under-reports; fix = wire `CollectionVisibilityModal` to the already-built `getCollectionPublishPreview` server endpoint (authoritative) and retire the now-unused preview route/wrapper/`countPublishableTrees`; (4) `freezeCollectionLinks` is also invoked from the collection-visibility route where it is a no-op (a collection going private changes no source tree's visibility) — verify and remove; (5) trim write-only `sourceWorkspaceId`/`sourceTreeId` on `PublicTreeRef` and the now-vestigial `familyName` prop on `PublishFlowContainer`.
   - **Chunk 4 cleanup — ✅ DONE (2026-06-18).** All five deferred items closed. (1) `MAX_EXTRA_TREES_PER_WORKSPACE` now enforced on both copy paths via a shared `src/lib/collections/extra-tree-cap.ts` (`assertExtraTreeCapacity`, counted against the TARGET workspace) — a copy can no longer bypass the cap. (2) The borrowed-branch copy was found to be already atomic in committed code (create + deep-copy + `CopyProvenance` in one `$transaction`) — no crash-window; no change needed. (3) `CollectionVisibilityModal` now fetches the authoritative `publish-preview` endpoint and renders the server `withheldTrees` count (which applies the cross-workspace `allowReuse` gate the old client rule omitted); the preview route + `countPublishableTrees` are now live-consumed rather than retired. (4) The no-op `freezeCollectionLinks` call was removed from the collection-visibility route (the real freeze-repoint still runs from the tree going-private path). (5) Unused `PublicTreeRef.sourceWorkspaceId`/`sourceTreeId` and the vestigial `PublishFlowContainer.familyName` prop trimmed. Security-reviewed: all-clear, no findings; the cross-tenant copy + encryption boundary intact. **Collections is now fully complete — no outstanding cleanups.**
...
- **DEFERRED to after deployment (owner decision, 2026-06-18).** Not a launch blocker for now, but to be enforced server-side before a true public push. **Open question to settle when this is built:** should the hide-birth-date enforcement apply to *every* public level, or only some — i.e. hide only when the tree is link-only (`public_link`), or only when it's search-engine-discoverable (`public_listed`), or always? (Note: the separate "born >130 years ago ⇒ treated as deceased, full date shown" living rule in §7.2 already runs server-side in the public serve path; this item is about the workspace's explicit per-gender hide-birth-date toggle, which is still client-only.)
...
- The **default indexable surface is the public tree page itself**, and there is still no separate family overview page. **Amended since:** per-person pages now exist at `/family/[slug]/person/[id]` for every non-private person on a published tree. They are kept out of search by default; a per-tree checkbox, off unless the owner ticks it, is what lets them be indexed.
...
- **Privacy posture (amended):** the original reasoning was that with no per-person pages a living person never becomes their own Google-rankable result. Per-person pages shipped, so that protection is now carried by the **opt-in instead of by absence**: a person page is out of search unless the tree is "findable in Google" **and** the owner ticked the per-tree checkbox, which stays off until they do. With the checkbox off the posture is unchanged — at most a name in a list on one page, birth date already hidden. With it on, the owner has deliberately asked for their family to be findable person by person, and the same redaction still applies (private people have no page at all, and the structured data carries no dates).
