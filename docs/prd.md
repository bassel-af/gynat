...
- **Product rules** (owner decisions, 2026-09-22 — do not re-open):
  ...
  - Optional generation range (min / max — either or both may be empty). No certainty label (صحيح/مقبول/…) — not asked for.
  - **Per-workspace opt-in, off by default** (owner decision 2026-09-23, superseding the earlier "no workspace toggle"): an admin turns «قفزة نسب» on under «المميزات» in the workspace settings. Off, the button to add a new jump is hidden and the server refuses to create one; existing jumps stay visible everywhere and can still be edited or deleted — «إيقاف الميزة لا يحذف الروابط المضافة». Imported GEDCOM jumps are kept regardless.
  ...
  - **Family name in the name chain** (owner decisions 2026-09-23): the person-page heading (member and public) never shows the family name — with or without a jump; it appears only in the «من بيت X» line. One-line names elsewhere keep the family name; when the chain crosses a jump it sits right before «، من وَلَد» («باسل بن محمد سعيد، من وَلَد عدنان»), never after the ancestor. The ancestor's own tribe/house name is never shown after the jump, and there is no warning when a chosen ancestor shares the family name.
