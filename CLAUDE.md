...
- Validation: `validateAddParent()`, `canMoveChild()`, `needsFamilyPickerForAddChild()`, `getAncestryJumpAction()` (`'create' | 'edit' | null` — «قفزة نسب» is offered only when `canEdit`, not `_pointed`, and the person has no `familyAsChild`)
...
- `undo-builders.ts` / `undo-label.ts` — inverse builders incl. `buildCreate/Update/DeleteAncestryJumpInverse`; label «قفزة نسب إلى {name}»
