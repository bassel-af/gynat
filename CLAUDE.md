...
- `PATCH /collections/[collectionId]/visibility` — admin-only; set collection visibility (`private`/`public_link`/`public_listed`); mints the public slug on first publish, keeps it across private round-trips. NO type-to-confirm phrase (each tree inside was already confirmed at its own publish). Going-private also runs `freezeCollectionLinks` (currently a no-op from this call site — flagged cleanup)
- `GET /collections/[collectionId]/publish-preview` — admin-only; the would-be-withheld trees + publishable count (currently NOT consumed by the UI — flagged cleanup; the modal computes withholding client-side)
...
- `/collections/[slug]` — public, no-login viewer for a published collection (Collections Chunk 4); deny-by-default (private/unknown/`enableCollections`-off → 404), always `noindex`; a brand-faithful (`/design-preview` vocabulary) card grid of the collection's public trees, each card opening that tree's own `/family/[slug]` viewer (`PublicCollectionPageClient`)
