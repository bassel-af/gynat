...
**Collections API Routes** (`src/app/api/workspaces/[id]/`) — all gated by `requireCollectionsEnabled` (404 when off) then the auth guard; Chunks 1–3 shipped (own-content, edit-content, add-by-link); public collection serving is Chunk 4:
...
- `/workspaces/[slug]/collections` + `/workspaces/[slug]/collections/[collectionId]` — collections list + detail (requires `enableCollections`): create collections, add own trees/branches as items, nest collections, add another family's tree/branch via link (public-slug or private share code; linked or copied), and publish a single extra tree (make-public ladder). Full public collection serving is Chunk 4
